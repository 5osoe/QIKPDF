...
let state = {
    ...
    rotateAngle: 90,
    splitFile: null,
    splitDoc: null, // pdfjsLib doc for preview/validation
    splitMode: 'all', // 'all' or 'custom'
    splitRanges: []
};
...
async function openPdfPreview(file) {
    ...
    // Ensure nav buttons are visible (reset from split preview state)
...
async function openSplitSinglePagePreview(pageNum) {
    ...
    try {
        ...
        const viewport = page.getViewport({ scale: 1.5 }); // Higher scale for modal view
        const canvas = DOM.pdfPreviewCanvas;
        const ctx = canvas.getContext('2d');
...
DOM.btnSplitAll.addEventListener('click', () => {
    state.splitMode = 'all';
    DOM.splitUi.classList.add('hidden');
    DOM.progressContainer.classList.remove('hidden');
...
DOM.btnProcessSplit.addEventListener('click', () => {
    // Gather ranges
    state.splitRanges = [];
...
function renderSplitRanges(count) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        // Click-to-Expand Preview Handlers
...
async function execSplit(file) {
    ...
    // --- MODE 2: CUSTOM RANGES ---
        ...
        for (let i = 0; i < total; i++) {
            ...
            const startIdx = range.start - 1; // 0-based
            const endIdx = range.end - 1;
            
            // Generate array of indices for the range
            const indices = [];
        ...
        state.resultName = file.name.replace(/\.pdf$/i, '_ranges.zip');
    }
