...
let state = {
    ...
    splitRanges: [],
    exportSettings: {
        size: 'default', // default, a4, a3, etc.
        filename: '',
        customWidth: 0,
        customHeight: 0
    }
...
DOM.btnProcess.addEventListener('click', async () => {
    ...
    if (tool === 'split') {
        if (state.splitMode === 'custom' && state.splitRanges.length === 0) {
             // Validate range logic if user didn't click anything yet
             // Actually ranges are gathered on process, so let's validate now
    ...
    // Trigger Export Modal (except for pdf2img which typically doesn't resize pages in the same way, but let's allow filename)
    // Actually pdf2img uses zipping images, page size concept applies less unless we resize images.
    // Requirement says "Default -> export unchanged". 
    // Let's Skip modal for pdf2img for simplicity as it produces ZIP of images.
    ...
    if (confirmed) {
        DOM.globalActions.classList.add('disabled-ui'); // Visual disable
...
function handleFiles(files) {
    ...
    // For single file tools, hide dropzone
    DOM.dropZone.classList.add('hidden');
    ...
    // PDF2IMG - Show generic ready state (no extra UI)
    if (state.currentTool === 'pdf2img') {
        DOM.statusText.innerText = "READY TO PROCESS";
    }
}
...
function renderReorderUI() {
    ...
    if (activeFiles.length === 0) {
        ...
        // If empty, show dropzone again
        DOM.dropZone.classList.remove('hidden');
        return;
    }
    
    // If we have files, hide dropzone (unless we want to allow adding more? 
    // Logic says "DROP FILE(S) HERE OR CLICK" is separate. 
    // Usually reorder UI allows adding more. 
    // For now, let's keep dropzone accessible or handle it. 
    // The previous implementation hid dropzone in some cases.
    // Let's show reorder container AND dropzone for multiple files.
    DOM.dropZone.classList.remove('hidden');
    ...
    // Removed the "reorder-actions" block (Clear/Process) from here as they are now Global.
...
async function processFiles() {
    // Note: files are already in state.
    const tool = state.currentTool;
    let files = [];
    if(tool === 'pdf2img' || tool === 'split' || tool === 'rotate') {
        // Files are stored individually
        if(tool === 'pdf2img' || tool === 'split') files = [state.splitFile || document.getElementById('file-input').files[0]]; 
        // Note: For pdf2img, we didn't store in state explicit var other than what handleFiles passed.
        // Actually handleFiles passed to processFiles directly before. 
        // Now we need to retrieve it.
        // For pdf2img, let's store it in splitFile (conceptually single file) or just access via file input if not drag/drop.
        // Fix: Update handleFiles to store single file for pdf2img too.
        if (tool === 'pdf2img') {
             // We need to ensure we have the file. 
             // Since we stopped auto-process, we must store it.
             // Let's use state.splitFile as a generic single file holder or create state.sourceFile
    ...
    // Safety fix for pdf2img if sourceFile wasn't set (because we modified handleFiles)
    if (tool === 'pdf2img' && !files[0]) {
         // handleFiles didn't set a state var for pdf2img. Let's fix handleFiles or just grab from re-architecture.
         // Let's modify handleFiles to set state.sourceFile for single file tools.
...
// Fix handleFiles to store sourceFile for pdf2img
...
handleFiles = function(files) {
    ...
    state.sourceFile = fileArr[0]; // Generic storage for single file tools
    _origHandleFiles(files);
}
