...
const UI_STRINGS = {
    ...
    split: "تقسيم",
    rotate: "تدوير",
    dropText: "أسقط الملف(ات) هنا<br>أو انقر للاختيار",
    toolTitle: "الأداة",
    metaTitle: "العنوان",
    metaAuthor: "المؤلف",
    metaSubject: "الموضوع",
    ...
    metaCreator: "المنشئ",
    clearMeta: "مسح كل البيانات الوصفية",
    selectAll: "تحديد الكل",
    selectNone: "إلغاء التحديد",
    searchPage: "رقم الصفحة",
    rotateLeft: "يسار",
    rotateFlip: "قلب",
    rotateRight: "يمين",
    ccw: "90° عكس عقارب",
    ...
    file: "ملف",
    pages: "صفحات",
    splitModeAll: "كل الصفحات (ملفات منفصلة)",
    splitModeCustom: "نطاقات مخصصة",
    outputFiles: "عدد ملفات الإخراج",
    from: "من",
    to: "إلى",
    filename: "اسم الملف",
    process: "معالجة PDF",
...
async function safetyGate(tool, files) {
    ...
    if (estMemory < (limit * 1.5)) {
        return confirm(
            `⚠️ تحذير الأداء\n\n` +
            `تتطلب هذه العملية حوالي ${Math.round(estMemory)} ميغابايت من الذاكرة.\n` +
    ...
    alert(
        ...
        `الذاكرة المقدرة: ${Math.round(estMemory)} ميغابايت\n` +
        `الحد الآمن: ${limit} ميغابايت\n\n` +
        `يرجى استخدام ملفات أصغر أو جهاز كمبيوتر مكتبي.`
    );
    return false;
}
...
const toolConfig = {
    'pdfinfo':  { title: 'معلومات PDF',       accept: 'application/pdf',          multiple: false },
    'metadata': { title: 'البيانات الوصفية',   accept: 'application/pdf',          multiple: false },
    'pdf2img':  { title: 'PDF ← صورة',        accept: 'application/pdf',          multiple: false },
    'img2pdf':  { title: 'صورة ← PDF',        accept: 'image/png, image/jpeg',    multiple: true  },
    'merge':    { title: 'دمج',               accept: 'application/pdf',          multiple: true  },
    'split':    { title: 'تقسيم',             accept: 'application/pdf',          multiple: false },
    'rotate':   { title: 'تدوير',             accept: 'application/pdf',          multiple: false }
};
...
function formatFileSize(bytes) {
    ...
    return (bytes / 1048576).toFixed(2) + " ميغابايت";
}
...
async function init() {
    try {
        ...
        console.error("فشل تحميل content.json");
    }
...
DOM.btnInfo.addEventListener('click', () => {
    ...
    const infoText = appContent.tools[jsonKey] || "المعلومات غير متاحة.";
    showInfoModal(`معلومات ${toolConfig[state.currentTool].title}`, infoText);
});
...
async function openEditPreview(data, type, pageNum = 1) {
    ...
    if (type === 'image') {
        ...
        try {
            ...
            DOM.previewBody.innerText = "خطأ في المعاينة";
        }
    }
    else if (type === 'pdf-full') {
        try {
            DOM.previewBody.innerHTML = '<div class="loading-spinner">جارٍ التحميل...</div>';
            let loadingTask;
    ...
    if (confirmed) {
        ...
        if (state.exportSettings.size === 'custom') {
            ...
            if (isNaN(w) || isNaN(h) || w <= 0 || h <= 0) {
                alert("يرجى إدخال أبعاد موجبة وصحيحة.");
                return;
            }
            let factor = 1;
...
DOM.btnProcess.addEventListener('click', async () => {
    ...
    try {
        ...
        if (tool === 'img2pdf') {
            ...
            if (activeFiles.length === 0) return alert("يرجى اختيار صور.");
        } else if (tool === 'merge') {
            activeFiles = state.mergeFiles;
            if (activeFiles.length === 0) return alert("يرجى اختيار ملفات PDF.");
        } else {
        ...
        if (tool === 'pdf2img') {
            ...
            if (state.pdfImgSelectedPages.size === 0) return alert("حدد صفحة واحدة على الأقل.");
        }
        ...
        if (tool === 'split' && state.splitMode === 'custom') {
            if (state.splitRanges.length === 0) {
                ...
                if (!valid || state.splitRanges.length === 0) return alert("نطاقات التقسيم غير صحيحة.");
            }
        }
...
function handleFiles(files) {
    ...
    if (oversized.length > 0) {
        ...
        errEl.innerHTML = `
            ...
            <div class="val-msg">
                <strong>الملف كبير جداً</strong>
                ${oversized.map(f => `${f.name} (${(f.size / 1048576).toFixed(1)} ميغابايت)`).join(', ')}
                يتجاوز الحد الآمن البالغ ${HARD_LIMIT_MB} ميغابايت لهذا الجهاز.
                يرجى استخدام ملف أصغر لتجنب تعطل المتصفح.
            </div>
        `;
    ...
    if (fileArr.length === 0) {
        alert("صيغة الملف غير مدعومة لهذه الأداة.");
        return;
    }
...
function setProgress(percent, msg = "جارٍ المعالجة...") {
