...
document.addEventListener("DOMContentLoaded", () => {
    initTheme(); 
    initLanguage(); // New Language Init
...
const TRANSLATIONS = {
    ...
    ar: {
        ...
        dropText: "أفلت الملفات هنا<br>أو انقر للاختيار",
        toolTitle: "الأداة",
        metaTitle: "العنوان",
        metaAuthor: "المؤلف",
        metaSubject: "الموضوع",
        ...
        metaCreator: "المنشئ",
        clearMeta: "مسح البيانات",
        selectAll: "تحديد الكل",
        selectNone: "إلغاء التحديد",
        searchPage: "بحث عن صفحة #",
        rotateLeft: "يسار",
        rotateFlip: "قلب",
        rotateRight: "يمين",
        ccw: "90° عكس الساعة",
        ...
        file: "الملف",
        pages: "الصفحات",
        splitModeAll: "الكل (ملفات منفصلة)",
        splitModeCustom: "نطاقات مخصصة",
        outputFiles: "عدد الملفات الناتجة",
        from: "من",
        to: "إلى",
        filename: "اسم الملف",
        ...
        clear: "مسح",
        success: "تم بنجاح!",
        ready: "ملفك جاهز للتحميل.",
        applyChanges: "تطبيق التغييرات",
        exportSettings: "إعدادات التصدير",
        exportFilename: "اسم الملف (اختياري)",
        pageSize: "حجم الصفحة",
        originalSize: "الأصلي (بدون تغيير)",
        customSize: "حجم مخصص...",
        docRotation: "تدوير المستند",
        none: "لا يوجد (0°)",
        cancel: "إلغاء",
        confirm: "تأكيد",
        width: "العرض",
        ...
        infoTitle: "معلومات",
        viewPdf: "عرض الملف",
        properties: "خصائص الملف",
        metaHeader: "البيانات الوصفية",
        fileNameLabel: "اسم الملف",
        fileSizeLabel: "الحجم",
        pagesLabel: "الصفحات",
        dimensionsLabel: "الأبعاد (ص1)",
        error: "خطأ",
...
function applyLanguage(lang) {
    // STRICT LTR: We do NOT set 'dir' attribute.
    // We only set data-lang to handle Font switching in CSS.
    document.body.setAttribute('data-lang', lang);
    
    // Update HTML Lang attribute for metadata
    document.documentElement.setAttribute('lang', lang);
    
    DOM.btnLang.innerText = lang === 'en' ? 'AR' : 'EN';
    updateUIText();
}
...
async function openEditPreview(data, type, pageNum = 1) {
    ...
    if (type === 'image') {
        ...
        try {
            ...
            // If data is a File object, read it, otherwise assume it's already a doc or arraybuffer
            let loadingTask;
            if (data instanceof File) {
                ...
                loadingTask = pdfjsLib.getDocument(data);
                 ...
                 // Assume it's a doc proxy if passed directly (rare here)
...
function openExportModal() {
    return new Promise((resolve) => {
        ...
        if (state.currentTool === 'img2pdf' || state.currentTool === 'merge') {
            ...
            // Rotation removed for export modal on these tools
        } 
...
DOM.toolCards.forEach(card => {
    card.addEventListener('click', () => {
        ...
        DOM.toolTitleText.innerText = getT(tool); // Translated title
        ...
        resetToolUI();
        updateUIText(); // Ensure correct language on view switch
    });
});
...
function renderSplitRanges(count) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        block.innerHTML = `
            ...
            <input type="text" class="range-name-input input-text" placeholder="${getT('filename')}" value="${storedName}" data-idx="${i}">
...
async function initPdfInfoUI(file) {
    ...
    try {
        ...
        let html = `
            <div class="info-panel">
                <div class="info-header-block">
                    <div class="info-filename">${file.name}</div>
                    <div class="info-filesize">${formatFileSize(file.size)}</div>
                </div>
                
                <h4 style="margin-bottom:12px; color:var(--accent); font-size:14px; letter-spacing:1px;">${getT('properties')}</h4>
                <div class="info-grid">
                    ${createRow('pagesLabel', doc.numPages)}
                    ${createRow('dimensionsLabel', formatPageSize(widthPt, heightPt))}
                </div>
                
                <h4 style="margin-bottom:12px; color:var(--accent); font-size:14px; letter-spacing:1px;">${getT('metaHeader')}</h4>
                <div class="info-grid">
                    ${createRow('metaTitle', info.Title)}
                    ${createRow('metaAuthor', info.Author)}
                    ${createRow('metaSubject', info.Subject)}
                    ${createRow('metaKeywords', info.Keywords)}
                    ${createRow('metaCreator', info.Creator)}
                </div>
        ...
        console.error(e);
        DOM.pdfInfoContent.innerHTML = `<div class="error-text">${getT('errorGeneric')}</div>`;
    }
}
    ...
    DOM.resultContainer.innerHTML = `
        <div class="success-box error-box">
            ...
            <div class="success-text">
                <h4>${getT('error')}</h4>
                <p>${e.message || getT('errorGeneric')}</p>
            </div>
        </div>
    `;
