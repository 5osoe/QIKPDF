...
DOM.btnProcess.addEventListener('click', async () => {
    ...
    // Modal Trigger
    let confirmed = true;
    // PDF2IMG doesn't typically resize, so we skip the resize modal but still allow processing
    // However, if we want to allow filename change, we could show it.
    // For consistency with previous phase, we skip it for pdf2img as per original logic requirement, 
    // unless refined. Let's keep it consistent: no export modal for pdf2img to keep flow fast.
    if (tool !== 'pdf2img') {
