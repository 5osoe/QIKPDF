...
const DOM = {
    ...
    // RESIZE UI
    resizeUi: document.getElementById('resize-ui'),
    resizeFilename: document.getElementById('resize-filename-display'),
    resizePageCount: document.getElementById('resize-pagecount'),
    resizeSelect: document.getElementById('resize-select'),
    resizeCustomGroup: document.getElementById('resize-custom-group'),
    resizeWidth: document.getElementById('resize-width'),
    resizeHeight: document.getElementById('resize-height'),
    resizeNameInput: document.getElementById('resize-name-input'),
...
const toolConfig = {
    ...
    'resize': { title: 'RESIZE', accept: 'application/pdf', multiple: false },
...
DOM.btnInfo.addEventListener('click', () => {
    ...
    const toolKeyMap = {
        'pdfinfo': 'pdfInfo', 'metadata': 'metadata', 'resize': 'resize',
...
async function openEditPreview(data, type, pageNum = 1) {
    ...
    if (type === 'image') {
        ...
        try {
            ...
            console.error("Full PDF load failed", e);
            DOM.previewBody.innerText = "Preview Error";
        }
    }
}
...
async function renderFullPdfPreviewPage() {
    ...
    navDiv.innerHTML = `
        ...
        <span>${previewState.page} / ${previewState.total}</span>
        <button id="next-btn" class="btn-outline">&gt;</button>
    `;
    DOM.previewBody.appendChild(navDiv);
    ...
    DOM.previewBody.querySelector('#prev-btn').onclick = () => {
        if(previewState.page > 1) { previewState.page--; renderFullPdfPreviewPage(); }
    };
    DOM.previewBody.querySelector('#next-btn').onclick = () => {
...
function openExportModal() {
    return new Promise((resolve) => {
        ...
        if (state.currentTool === 'img2pdf' || state.currentTool === 'merge') {
            ...
            if(DOM.exportRotationGroup) DOM.exportRotationGroup.classList.remove('hidden');
        } 
...
DOM.btnProcess.addEventListener('click', async () => {
    try {
        ...
        if (tool === 'resize') {
            const filename = DOM.resizeNameInput.value.trim();
            if(!filename) return alert("Please enter a filename.");
            
            const size = DOM.resizeSelect.value;
            if (size === 'custom') {
                 const w = parseFloat(DOM.resizeWidth.value);
                 const h = parseFloat(DOM.resizeHeight.value);
                 if (isNaN(w) || isNaN(h) || w <= 0 || h <= 0) return alert("Invalid dimensions.");
            }
        }

        // For resize tool, we skip the standard modal because inputs are in UI
        let confirmed = false;
        if (tool === 'resize') {
            confirmed = true;
        } else {
            confirmed = await openExportModal();
        }
        ...
        if (confirmed) {
            ...
            DOM.resizeUi.classList.add('hidden');
...
// Metadata Tool Specific Logic
DOM.btnMetaClear.addEventListener('click', () => {
    DOM.metaTitle.value = '';
    DOM.metaAuthor.value = '';
    DOM.metaSubject.value = '';
    DOM.metaKeywords.value = '';
    DOM.metaCreator.value = '';
});

// Resize Tool Specific Logic
DOM.resizeSelect.addEventListener('change', (e) => {
    if(e.target.value === 'custom') DOM.resizeCustomGroup.classList.remove('hidden');
    else DOM.resizeCustomGroup.classList.add('hidden');
});
...
function resetToolUI() {
    ...
    // Resize Reset
    DOM.resizeNameInput.value = '';
    DOM.resizeSelect.value = 'a4';
    DOM.resizeCustomGroup.classList.add('hidden');
...
function renderSplitRanges(count) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        startCanvas.addEventListener('click', () => {
            ...
            if (val) openEditPreview(state.splitDoc, 'pdf-single', val);
        });
...
async function initPdfInfoUI(file) {
    ...
    DOM.globalActions.classList.add('hidden'); // No process action required
    DOM.pdfInfoContent.innerHTML = '<div class="loading-spinner">READING PDF INFO...</div>';
    ...
    try {
        ...
        let html = `
            <div class="info-panel">
                ...
                ${createRow('FILE NAME', file.name)}
                ${createRow('FILE SIZE', fileSizeKB + ' KB')}
                ${createRow('PAGES', doc.numPages)}
                ${createRow('DIMENSIONS (P1)', `${width} x ${height} pt`)}
                
                <h4 style="margin-top:24px;">METADATA</h4>
                ${createRow('TITLE', info.Title)}
                ${createRow('AUTHOR', info.Author)}
                ${createRow('SUBJECT', info.Subject)}
                ${createRow('KEYWORDS', info.Keywords)}
                ${createRow('CREATOR', info.Creator)}
                ${createRow('PRODUCER', info.Producer)}
                ${createRow('PDF VERSION', info.PDFFormatVersion)}
            </div>
        `;
        ...
        console.error(e);
        DOM.pdfInfoContent.innerHTML = '<div class="error-text">FAILED TO READ PDF INFO</div>';
    }
}

async function initMetadataUI(file) {
    DOM.metadataUi.classList.remove('hidden');
    DOM.btnProcess.innerText = "APPLY CHANGES";
    DOM.metaTitle.value = "LOADING...";
    DOM.metaAuthor.value = "LOADING...";
    ...
    try {
        ...
        console.error(e);
        alert("Failed to read existing metadata.");
    }
}

async function initResizeUI(file) {
    DOM.resizeUi.classList.remove('hidden');
    DOM.resizeFilename.innerText = file.name;
    DOM.resizePageCount.innerText = "LOADING...";
    
    try {
        const arrayBuffer = await file.arrayBuffer();
        const loadingTask = pdfjsLib.getDocument({ data: arrayBuffer });
        const doc = await loadingTask.promise;
        DOM.resizePageCount.innerText = doc.numPages + " PAGES";
        doc.destroy();
    } catch(e) {
        DOM.resizePageCount.innerText = "ERROR";
    }
}
...
// --- NEW SEARCH LOGIC (REPLACES JUMP) ---
DOM.pdfImgSearch.addEventListener('input', (e) => {
...
function handleFiles(files) {
    ...
    if (state.currentTool === 'pdfinfo') {
        initPdfInfoUI(fileArr[0]);
        return;
    }
    if (state.currentTool === 'metadata') {
        initMetadataUI(fileArr[0]);
        return;
    }
...
function renderReorderUI() {
    ...
    activeFiles.forEach((file, index) => {
        ...
        if (isImg) {
            ...
            // BIND PREVIEW
            thumb.onclick = () => openEditPreview(file, 'image');
            ...
            // BIND PREVIEW
            docIcon.onclick = () => openEditPreview(file, 'pdf-full');
...
async function processFiles() {
    ...
    if(tool === 'pdf2img' || tool === 'split' || tool === 'rotate' || tool === 'metadata' || tool === 'resize') {
    ...
    else if (tool === 'resize') await execResize(files[0]);
    
    DOM.progressContainer.classList.add('hidden');
    DOM.resultContainer.classList.remove('hidden');
    
    // Final Result Action - Success Feedback
    UrlManager.revokeAll();

    // IMPROVED SUCCESS BLOCK
    DOM.resultContainer.innerHTML = `
...
// --- CORE PROCESSING LOGIC (UNTOUCHED / EXTENDED) ---
...
async function execResize(file) {
    const { PDFDocument } = PDFLib;
    setProgress(0, "LOADING PDF...");
    await yieldToMain();
    const arrayBuffer = await file.arrayBuffer();
    ...
    if (size === 'custom') {
        // MM to Points (1 mm = 2.83465 pt)
        targetW = parseFloat(DOM.resizeWidth.value) * 2.83465;
        targetH = parseFloat(DOM.resizeHeight.value) * 2.83465;
    } else {
        [targetW, targetH] = PAGE_SIZES[size];
    }
    
    const newPdf = await PDFDocument.create();
    const embeddedPages = await newPdf.embedPages(pdf.getPages());
    const total = embeddedPages.length;
    ...
    for (let i = 0; i < total; i++) {
        ...
        const { width, height } = embPage; // Original dimensions
        ...
        page.drawPage(embPage, { x, y, width: drawW, height: drawH });
    }
    ...
    let filename = DOM.resizeNameInput.value.trim();
...
async function execRotate(file) {
    ...
    if (sizeSetting === 'default') {
        ...
        for (let i = 0; i < total; i++) {
            ...
            // STRICT CONTAINER MODE: Fixed dimensions
