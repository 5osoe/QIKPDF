...
async function openPdfPreview(file) {
    ...
    try {
        ...
        pdfPreviewDoc = await pdfjsLib.getDocument(url).promise;
        pdfPreviewPageNum = 1;
...
DOM.btnPdfPrev.addEventListener('click', () => {
    if (pdfPreviewDoc && pdfPreviewPageNum > 1) {
        pdfPreviewPageNum--;
        renderPdfPreviewPage(pdfPreviewPageNum);
    }
});
...
DOM.btnPdfNext.addEventListener('click', () => {
    if (pdfPreviewDoc && pdfPreviewPageNum < pdfPreviewDoc.numPages) {
        pdfPreviewPageNum++;
        renderPdfPreviewPage(pdfPreviewPageNum);
    }
});
...
// --- Drag & Drop Reordering UI (Refactored to CSS Classes) ---
