...
// --- MEMORY MANAGEMENT SYSTEM (Repair Phase) ---
const UrlManager = {
    urls: new Set(),
    create: function(obj) {
...
function closeModal(modalElement) {
    ...
    // Check if any other modals are open before unlocking scroll
    const anyOpen = document.querySelectorAll('.modal:not(.hidden)').length > 0;
    if (!anyOpen) {
        document.body.classList.remove('modal-open');
    }

    // Specific cleanup for previews
    if (modalElement === DOM.imgPreviewModal) {
        DOM.imgPreviewElement.src = '';
    }
    if (modalElement === DOM.pdfPreviewModal) {
        cleanupPdfPreview();
    }
}
...
// Edit Button Logic (Restores Input UI)
DOM.btnEditResult.addEventListener('click', () => {
    DOM.resultContainer.classList.add('hidden');
    DOM.progressContainer.classList.add('hidden');

    // Restore UI based on tool
    if (state.currentTool === 'img2pdf' || state.currentTool === 'merge') {
...
DOM.btnResetTool.addEventListener('click', goHome); // Header "X" goes Home
...
async function processFiles() {
    ...
    // Swap Footer Buttons (Edit | Download | Clear)
