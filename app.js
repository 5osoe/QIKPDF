...
function closeModal(modalElement) {
    ...
    if (modalElement === DOM.imgPreviewModal) DOM.imgPreviewElement.src = '';
    if (modalElement === DOM.pdfPreviewModal) cleanupPdfPreview();
}
...
DOM.btnEditResult.addEventListener('click', () => {
    ...
    // Re-create input thumbnails if necessary
    if (state.currentTool === 'img2pdf' && state.imgFiles.length > 0) renderReorderUI();
...
function openImgPreview(src, title) {
    ...
    openModal(DOM.imgPreviewModal);
}
DOM.btnImgPreviewClose.addEventListener('click', () => closeModal(DOM.imgPreviewModal));
...
async function openPdfPreview(fileOrBlob) {
    cleanupPdfPreview();
    DOM.btnPdfPrev.classList.remove('hidden');
    DOM.btnPdfNext.classList.remove('hidden');

    DOM.pdfPreviewTitle.innerText = fileOrBlob.name || "PREVIEW";
    openModal(DOM.pdfPreviewModal);
...
DOM.btnPdfPreviewClose.addEventListener('click', () => closeModal(DOM.pdfPreviewModal));
...
async function renderFinalResultPreview(blob, tool) {
    ...
    // Revoke previous result preview URLs
    UrlManager.revokeAll();
    ...
    if (blob.type === 'application/pdf') {
        ...
        try {
            ...
            if (tool === 'pdf2img') {
                ...
                // Extract images to show structured preview
                const promises = [];
                zip.forEach((relativePath, zipEntry) => {
                    if (!zipEntry.dir && zipEntry.name.match(/\.png$/i)) {
                        promises.push(zipEntry.async('blob').then(b => ({blob: b, name: zipEntry.name})));
                    }
                });
                
                // Limit for performance or show all? 
                // Showing all for correctness, but lazy loading ideally. 
                // For safety, let's show first 20.
                const images = await Promise.all(promises);
                images.sort((a,b) => a.name.localeCompare(b.name));
                
                images.slice(0, 50).forEach(imgData => {
                    const url = UrlManager.create(imgData.blob);
                    
                    const card = document.createElement('div');
                    card.className = 'result-card';
                    card.innerHTML = `
                        <img class="result-thumb" src="${url}">
                        <input class="result-filename" value="${imgData.name}">
                    `;
                    card.querySelector('img').onclick = () => openImgPreview(url, imgData.name);
                    DOM.resultGrid.appendChild(card);
                });

                DOM.resultZipInfo.classList.remove('hidden');
                DOM.resultZipName.innerText = `${images.length} IMAGES GENERATED`;
                ...
                for(const name of fileNames) {
                    ...
                    row.innerHTML = `
                        <input class="result-filename" value="${name}">
                        <div class="result-meta">PDF FILE</div>
                        <button class="preview-btn">PREVIEW</button>
                    `;
                    
                    row.querySelector('.preview-btn').onclick = () => {
                         openPdfPreview(new File([blob], name, {type: 'application/pdf'}));
                    };
