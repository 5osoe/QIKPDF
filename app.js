...
// --- PDF Viewer Modal (Lightweight & Memory Safe) ---
...
async function openPdfPreview(file) {
    // Cleanup any existing state before opening a new file
    closePdfPreview();
    ...
    try {
        // Create object URL and store it for later cleanup
...
async function renderPdfPreviewPage(num) {
    ...
    // Prevent rendering multiple pages at the exact same time
    if (pageRendering) {
        pageNumPending = num;
        return;
    }
    ...
    try {
        ...
        // Free page memory immediately after rendering
        page.cleanup();

        DOM.pdfPageCurrent.innerText = num;
        pageRendering = false;

        // Render any page that was requested while we were busy
...
// Strict Memory Safety & Cleanup function
