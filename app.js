...
async function openEditPreview(data, type, pageNum = 1) {
    ...
    if (type === 'image') {
        ...
        // data is pdfDoc proxy
        try {
            const page = await data.getPage(pageNum);
            const viewport = page.getViewport({ scale: 2 }); // Good quality preview
...
function closeEditPreview() {
    ...
    DOM.previewBody.innerHTML = '';
    // If we loaded a doc specifically for preview (merge), destroy it
    if(previewState.doc) {
...
async function execRotate(file) {
    ...
    if (sizeSetting === 'default') {
        ...
        for (let i = 0; i < total; i++) {
            ...
            // STRICT FIX: Swap container dimensions if rotation is sideways
            let [pgW, pgH] = target;
            if (rot % 180 !== 0) { pgW = target[1]; pgH = target[0]; }
            
            // Fix scale using actual dimensions
            ...
            // Use actual page dimensions for center
            const centerX = pgW / 2;
            const centerY = pgH / 2;
