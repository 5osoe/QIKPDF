...
let renderTask = null; // To cancel slow renders if user clicks Next quickly
...
async function openPdfPreview(file) {
    ...
    try {
        // FAST LOADING: Using ArrayBuffer instead of ObjectURL skips network simulation inside pdf.js
...
async function renderPdfPreviewPage(num) {
    ...
    if (pageRendering) {
        pageNumPending = num;
        // SPEED BOOST: Cancel ongoing render to load the new requested page instantly
        if (renderTask) {
            renderTask.cancel();
        }
        return;
    ...
    try {
        ...
        const viewport = page.getViewport({ scale: 1.2 }); // Optimized scale for speed
        ...
        // Ignore errors caused by our intentional render cancellation
        ...
        // Render any pending page immediately
