async function initSplitUI(file) {
    ...
    try {
        ...
        renderSplitAllNames();
    } catch (e) { DOM.splitPageCount.innerText = "ERROR"; }
}
...
async function renderSplitAllNames() {
    ...
    if (DOM.splitAllCount) DOM.splitAllCount.innerText = total + ' PAGES';
    ...
    for (let i = 1; i <= total; i++) {
        ...
        thumbBox.title     = 'Click to preview';
        const pageNum = document.createElement('span');
        pageNum.className = 'page-num';
        pageNum.textContent = `P${i}`;
...
function renderSplitRanges(count) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        startCanvas.addEventListener('click', () => { const val = parseInt(startInput.value); if (val && !processingLock) openEditPreview(state.splitDoc, 'pdf-single', val); });
        endCanvas.addEventListener('click',   () => { const val = parseInt(endInput.value);   if (val && !processingLock) openEditPreview(state.splitDoc, 'pdf-single', val); });
    }
}
